...
The exposition on this section is suported by Figure~\ref{usage}.
...
The software is then ready to render the network on the screen for arbitrary
navigation, but the user may wish to select one of the available network layout
algorithms and tune the coarsening to be performed.
Upon request given by hiting the ``render network'' button,
the network is mapped to the screen.
The subsequent usage relies on the manipulation of the visual mapping presented on the
canvas by means of actions
performed through managing the tools and related mouse operations.

There are multiple levels of representation, which is entailed by the multilevel strategy.
This makes pertinent that actions be performed in only one of the available levels.
Thus, there is always a ``level on focus'' in which the tools perform actions, such
as link coloring or opening supernodes to expose their child nodes.
The first button on the toolbar is thus for changing the level on focus.
The second set of buttons are for changing how the nodes are visualized:
color, size, if size is related to degree or number of children, restore node size,
node transparency, and rotation.
The third set of tool buttons if for links: color, thickness, transparency, if thickness and
transparency are related to link weight, and restor link properties.

The fourth set of tool buttons is more specific to the needs of the MlBiNetViz, and
...
id, number of children, id of parent, number of neighbors (degree), sum of weight of links (strength), and other information the analysts report necessity of.
The second tool button is seleted to move nodes: specific nodes are individually moved if clicked and dragged; a rectangular region is made available for moving all nodes therein if an empty region of the canvas is clicked and the mouse cursor dragged before releasing the button.
The third tool button is selected for opening a supernode and exposing their child nodes and links among them:
a region is defined by the user by clicking on an empty space on the canvas and gragging the mouse before releasing, the supernodes therein are joined into a rectangular region that contains all of them, and their child nodes (on a finer level of detail) are drawn inside such rectangular region.
...
The last tool button is selected to perform three operations on open supernodes:
...
This is most useful when used in combination with the drag tool: on the finer level of detail, the nodes are repositioned into a more compact disposition. Then upon a (potentially minimal) supernode
risize, the child nodes are made to fit the whole rectangular area.

The last set of tools acts on the whole canvas: they are dedicated to zoom and left-right, up-down pan, changing the color of the background, and toggling between current zoom and pan status and the initial setting.
...
and use the MlBiNetViz in the instance we made availabel~\cite{mbnvpage}.
The more seasoned software developer may browser the code, upload a local instance
...
specific or heavy calculations.
There is a secondery server, a node FeatherJS, used to facilitate contact with the database
...
MlBiNetViz. The interested reader is welcome to browse the software code and documentation
...
Also, it is the first contribution to bipartite networks visual analytics
assited by multilevel strategies.
The interface developed is efficiently managed through simple interactive operations.

The navigation may be enhanced by the use of in-development coarsening algorithms.
The visualization pathway should be used to assist multilevel strategies by enabling the user
to select nodes as pivots to guide the coarsening procedure.
Also, the interface is envioned to assist the analysis of coarsening results.
The bipartite framework is naturally extended to heterogeneous (or multipartite) networks,
which may be a research route we will follow if there is enough interest of the researchers involved.
